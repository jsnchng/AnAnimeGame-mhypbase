...
```powershell
git clone https://github.com/jsnchng/AnAnimeGame-mhypbase.git
```
```powershell
cd AnAnimeGame-mhypbase
```
Configure:
```powershell
...
```
## Usage
1. Backup the original `mhypbase.dll`.
2. Copy `mhypbase.ini` and `build/Release/mhypbase.dll` into the game directory.
3. Uncomment the `ConfigBaseUrl` and `ConfigChannel` entries in `mhypbase.ini` to configure it correctly.
## Legacy README
> # mhypbase
> 
